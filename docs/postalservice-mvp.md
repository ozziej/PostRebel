...
- [x] Collection Runner — visual flow-based runner built with React Flow; chain requests sequentially, pass data between them via dot-notation response mappings (`body.access_token → {{access_token}}`), branch with JavaScript conditions (`return status === 200`), view each node's response inline; runner files stored in `{workspace}/runners/` and committed to git
